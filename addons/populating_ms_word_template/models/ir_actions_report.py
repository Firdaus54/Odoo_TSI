            ...
            if len(childs) == 1:
                ...
                if childs[0] == 'line':
                    ...
                    if tmp_logic == 'sum':
                        ...
                        data = datas
                        for child in childs:
                            if isinstance(data, models.BaseModel) and len(data) > 1:
                                # Ambil field dari masing-masing record dan gabungkan string-nya
                                data_list = []
                                for rec in data:
                                    value = getattr(rec, child, '')
                                    data_list.append(str(value or ''))
                                data = ', '.join(data_list)  # atau bisa pakai '\n'.join()
                                break  # Keluar dari loop karena sudah selesai
                            else:
                                data = getattr(data, child)
                        simple_merge[field] = str(data or '')
