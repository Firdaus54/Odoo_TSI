    ...
    def create(self, vals):
        ...
        if res.provider_id and res.partner_id and res.partner_id.mobile:
            res.partner_id.write(
                {"mobile": res.partner_id.mobile.strip("+").replace(" ", "")}
            )
            part_lst = []
            ...
            if res.type == "received":
                ...
                if operators:
                    for operator in operators:
                        operator_channel = (
                            operator.partner_id.channel_provider_line_ids.channel_id
                        )
                        if operator_channel:
                ...
                if not self.env.context.get("whatsapp_application"):
                    if "template_send" in self.env.context and self.env.context.get(
                        ...
                        if wa_template.template_type == "interactive":
                                ...
                                if component.type == "interactive":
                                    if component.interactive_type == "product_list":
                                        ...
                                        if component.interactive_button_ids:
                                            ...
                                            for (
                                                btn_id
                                            ) in component.interactive_button_ids:
                                                buttons.append(
                                                    {
                                                        "type": "reply",
                                                        "reply": {
                                        ...
                                        if component.interactive_list_ids:
                                            ...
                                            for (
                                                list_id
                                            ) in component.interactive_list_ids:
                                                rows = []
                            ...
                            try:
                                answer = res.provider_id.send_mpm_template(
                                    wa_template.name,
                            ...
                            for component in wa_template.components_ids:
                                object_data = self.env[
                                    ...
                                    [
                                        (
                                            "id",
                                            "=",
                                            self.env.context.get("active_model_id"),
                                        )
                                    ]
                                )[
                                    0
                                ...
                                if component.type in ["body", "footer"]:
                                    ...
                                    for length, var in zip(
                                        range(variables_length), component.variables_ids
                                    ):
                                        ...
                                        if var.field_id.model:
                                            ...
                                            if vals.get("mail_message_id"):
                                                mail_message = mail_message.browse(
                                                    vals.get("mail_message_id")
                                                )
                                            if mail_message and isinstance(
                                                value, tuple
                                            ):
                                                value = value[1]
                                                mail_message.write(
                                                    {
                                                        "body": mail_message.body.replace(
                                                            st, str(value)
                                                        )
                                                    }
                                                )
                                ...
                                if component.type == "buttons":
                                    if (
                                        component.button_type == "call_to_action"
                                        and (
                                            component.type_of_action
                                            or component.type_of_action_2
                                        )
                                        == "URL"
                                        and (component.url_type or component.url_type_2)
                                        == "dynamic"
                                    ):
                                ...
                                if bool(template_dict):
                                    params.append(template_dict)
                        try:
                            answer = res.provider_id.send_template(
                                wa_template.name,
                                wa_template.lang.iso_code,
                                wa_template.namespace,
                                res.partner_id,
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    if self.env.context.get("wa_messsage_id"):
                                        self.env.context.get(
                                            "wa_messsage_id"
                                        ).wa_message_id = dict.get("messages")[0].get(
                                            "id"
                                        )
                            else:
                                if "sent" in dict and dict.get("sent"):
                        ...
                        if res.message:
                            ...
                            if "message_parent_id" in self.env.context:
                                ...
                                answer = res.provider_id.send_message(
                                    res.partner_id,
                                    res.message,
                        ...
                        if res.attachment_ids:
                            for attachment_id in res.attachment_ids:
                                ...
                                if res.provider_id.provider == "graph_api":
                                    ...
                                    answer = res.provider_id.send_image(
                                        res.partner_id, attachment_id
                                    )
                                    if answer.status_code == 200:
                                        dict = json.loads(answer.text)
                                        media_id = dict.get("id")
                                        getimagebyid = res.provider_id.get_image_by_id(
                                            media_id,
                                            res.partner_id,
                                            sent_type,
                                            attachment_id,
